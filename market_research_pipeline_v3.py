...
def read_goals(goals_csv: Path) -> pd.DataFrame:
    ...
    if goals.empty:
        return pd.DataFrame(columns=["goal_time"])
    time_col = None
    for c in ["goal_time", "publish_time", "pt_utc", "pt"]:
    ...
    if time_col is None:
        ...
        return pd.DataFrame(columns=["goal_time"])
    goals["goal_time"] = pd.to_datetime(goals[time_col], errors="coerce", utc=True)
    return goals.dropna(subset=["goal_time"]).sort_values("goal_time")
...
def pass1_catalog_and_sample(csv_path: Path, goals: pd.DataFrame, out_dir: Path, meta: Dict[str, object]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for i, chunk in enumerate(pd.read_csv(csv_path, chunksize=CHUNK_SIZE, low_memory=True)):
...
def pass2_event_study(csv_path: Path, goals: pd.DataFrame, out_dir: Path, meta: Dict[str, object]) -> None:
    ...
    parts = []
    for i, chunk in enumerate(pd.read_csv(csv_path, chunksize=CHUNK_SIZE, usecols=lambda c: c in cols_needed, low_memory=True)):
        chunk = parse_time_series(chunk, time_col)
        if chunk.empty:
            continue
