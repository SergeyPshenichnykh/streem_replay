...
# Друкувати тільки при реальній зміні стану
...
def update_book(book, levels):
    ...
    levels = список змін типу [[price, size], ...]
    
    Логіка:
    - якщо size > 0 -> оновити/додати рівень
    - якщо size == 0 -> видалити рівень
    """
        ...
        for line in f:
            ...
            for mc in data.get("mc", []):
                ...
                for rc in mc.get("rc", []):
                    ...
                    # LTP оновлюємо тільки якщо валідний
                    if "ltp" in rc:
                        p = rc.get("ltp")
                        if valid_price(p):
                            state["ltp"] = p

                    # TV оновлюємо тільки якщо валідний
                    if "tv" in rc:
                        tv = rc.get("tv")
                        if isinstance(tv, (int, float)) and tv >= 0:
                            state["tv"] = tv

                    # atb = delta змін back_book
                    if "atb" in rc:
                        update_book(state["back_book"], rc.get("atb"))

                    # atl = delta змін lay_book
                    if "atl" in rc:
