...
# Якщо True — друкувати тільки тоді, коли реально змінився стан ринку
...
def best_price_size(levels):
    # Бере ПЕРШИЙ ВАЛІДНИЙ рівень стакану
    # і відсікає сміття типу 1000 або size <= 0
...
def snapshot_signature(runner_names, runner_state):
    # Підпис поточного стану ринку.
    # Якщо підпис не змінився — можна не перемальовувати кадр.
    items = []
        ...
        for line in f:
            ...
            for mc in data.get("mc", []):
                ...
                for rc in mc.get("rc", []):
                    ...
                    # LTP оновлюємо тільки якщо значення валідне
                    if "ltp" in rc:
                        p = rc.get("ltp")
                        if valid_price(p):
                            state["ltp"] = p

                    # TV оновлюємо тільки якщо значення валідне
                    if "tv" in rc:
                        tv = rc.get("tv")
                        if isinstance(tv, (int, float)) and tv >= 0:
                            state["tv"] = tv

                    # BACK: беремо перший валідний рівень
                    if "atb" in rc:
                        bb_price, bb_size = best_price_size(rc.get("atb"))
                        if bb_price is not None and bb_size is not None:
                            state["best_back_price"] = bb_price
                            state["best_back_size"] = bb_size

                    # LAY: беремо перший валідний рівень
                    if "atl" in rc:
                        bl_price, bl_size = best_price_size(rc.get("atl"))
                        if bl_price is not None and bl_size is not None:
                            state["best_lay_price"] = bl_price
                            state["best_lay_size"] = bl_size

                # 3. Додатковий захист:
                # якщо BACK >= LAY, такий кадр ринку підозрілий.
                # У такому випадку нічого не ламаємо, просто не затираємо валідний стан.
                for runner_id, state in runner_state.items():
                    bb = state.get("best_back_price")
                    bl = state.get("best_lay_price")

                    if bb is not None and bl is not None:
                        if bb >= bl:
                            # підозрілий стан — лишаємо як є,
                            # але не робимо тут ніякого "ремонту"
                            pass
